...
const Carrito = () => {
    ...
    const { submitOrder } = useOrderHook();
    const {generateWhatsappTemplate} = useWhatsappHook()
    ...
    const handleSendOrder = async () => {
        ...
        const whatsappLink = generateWhatsappTemplate(cart, orderId);
                ...
                Object.keys(cart).length > 0 ? (
                    <div>
                        <ul className='max-h-[90%] overflow-y-scroll gap-2 grid xl:grid-cols-2'>
                            {
                                Object.values(cart).map(pr => (
                                    <li className='relative border-b border-gray-300' key={pr.product.id}>
                                        <div className='flex justify-between gap-2 p-2 items-center w-full'>
                                            ...
                                            <h2 className='text-xl w-[30%]'>{pr.product.name}</h2>
                        ...
                        <div className='flex justify-center py-4'>
                            <button onClick={handleSendOrder} className="p-3 border-none rounded-full bg-[#25D366] text-white text-lg font-medium flex items-center justify-center gap-2 shadow-md hover:bg-[#1ebe5d] transition-all duration-300">
                    ...
                    <div className='flex flex-col justify-center items-center h-full md:h-fit'>
                        <div className='flex gap-2 flex-wrap items-center justify-center'>
                            <h2 className='text-xl text-center text-gray-500 font-bold'>Aún no has agregado productos al carrito de compras</h2>
